# Nandini Iyer - My Additions:
## myWorld:
- I added many rooms, along with descriptions for each room 
- Each room displays a message every time the do something button is interacted with
    - I made sure that if there is no personalized message, it just displays "Interaction Succesful"
- Rooms before locked rooms can display different messages when the do something button is interacted with
    - Depends on how many requirements there are to enter each room
    - Say you need a notebook and a pen to enter- it will display different messages if you have only the pen, only the notebook, both, or neither

## Rules
- `hasRequirements`: Checks if the player has collected all the required items to move on to the next room
    - If not: it prevents the player from going to the next room while displaying a personalized message
        - Message found in myWorld
- `handleInteractions`: checks to see if the player has collected the necessary items to move beyond the rooms hidden behind puzzles
    - If so, displays a message
    - If not, displays a different message
    - Messages updated in myWorld, and depends on how many of the necessary items the player has collected 
- `Added puzzles`:
    - notebook + pen = can decode a lock and exit the building
    - hammer = can smash open a wall to reveal a hidden study
    - key = can unlock a door 
    - paper = has a code to unlock a door
    - flashlight + battery = allows the player to go through two dark rooms
- `Flashlight puzzle`: 
    - The player can go through the dark rooms without a flashlight but they won't be able to see anything
    - Only when the player has both the flashlight and the battery AND have turned it on in the previous room will they be able to see and interact with things in the next room
- `Scoring`: Takes the total number of interactions and divides it by the number of steps taken
    - multiplied by 100 for legibility 
    - Good way to measure how much they interacted with the game compared to how many steps they took
    - Encouranges interaction
